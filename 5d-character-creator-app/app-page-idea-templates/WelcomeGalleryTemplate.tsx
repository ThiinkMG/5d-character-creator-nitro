...
// =============================================================================
// PLACEHOLDER DATA
// TODO: Antigravity - Replace with actual data fetching from your state/API
// =============================================================================
...
const ImageGeneratorModal: React.FC<ImageGeneratorModalProps> = ({
  ...
  const [selectedProvider, setSelectedProvider] = useState<'free' | 'gemini' | 'dalle'>('free');
  ...
  const providers = [
    ...
    { 
      id: 'gemini' as const, 
      name: 'Google Gemini', 
      description: currentProvider.apiKeyConfigured ? 'API key configured' : 'Requires API key',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-2xl mx-4 rounded-2xl overflow-hidden"
        style={{
          ...
          boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.8), 0 0 60px rgba(249, 115, 22, 0.1)'
        }}
      >
        {/* Header */}
        <div className="p-6 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-white tracking-tight">Generate Image</h2>
        ...
        <div className="p-6 space-y-6">
                  ...
                  <button
                    ...
                    style={{
                      background: isSelected 
                        ...
                        : 'rgba(12, 12, 18, 0.6)',
                      border: `1px solid ${isSelected ? 'rgba(249, 115, 22, 0.4)' : 'rgba(255, 255, 255, 0.06)'}`,
                      boxShadow: isSelected ? '0 0 20px rgba(249, 115, 22, 0.1)' : 'none'
                    }}
                  >
                    <Icon className={`w-5 h-5 mb-2 ${isSelected ? 'text-[#F97316]' : 'text-white/50'}`} />
                    <div className="text-sm font-medium text-white">{provider.name}</div>
                    <div className="text-xs text-white/40 mt-1">{provider.description}</div>
          ...
          >
            ...
            <div className="text-sm text-white/60">
              ...
              <p>The free generator works without any setup. For higher quality results, 
              configure Gemini or DALL-E API keys in <a href="/settings" className="text-[#F97316] hover:underline">Settings → Image Generation</a>.</p>
            </div>
          </div>
        </div>
...
const GalleryCard: React.FC<GalleryCardProps> = ({ item, onClick, onImageChange }) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        minHeight: item.size === 'tall' ? '400px' : item.size === 'small' ? '180px' : '200px'
      }}
    ...
    >
      ...
      <div className="relative h-full flex flex-col justify-end p-4 md:p-5">
        ...
        <div className="absolute top-3 right-3">
          ...
          >
            {item.imageSource === 'generated' && <Sparkles className="w-3 h-3 text-[#F97316]" />}
            {item.imageSource === 'uploaded' && <Upload className="w-3 h-3 text-[#14B8A6]" />}
            {item.imageSource === 'preset' && <ImageIcon className="w-3 h-3 text-white/50" />}
          </div>
        </div>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 p-4">
        ...
        <div className="text-center">
          <p className={`text-sm font-medium transition-colors duration-300 ${
            ...
            Create New {type === 'character' ? 'Character' : 'World'}
          </p>
 ...
 * 1. Wire up actual data fetching to replace PLACEHOLDER_ITEMS
 * 2. Implement image upload handler (onImageChange with 'upload')
 * 3. Implement image generation API routes:
 *    - POST /api/generate-image (free generator)
 *    - POST /api/generate-image/gemini (requires API key)
 *    - POST /api/generate-image/dalle (requires API key)
 * 4. Update Settings page to include:
 *    - Image Generation section
 *    - Gemini API key input
 *    - DALL-E API key input
 *    - Provider preference toggle
 * 5. Add model switching logic in ImageGeneratorModal
 * 6. Implement image replace flow (update item.imageUrl and item.imageSource)
 * 7. Add loading states during image generation
 * 8. Add error handling for failed generations
 */
export const WelcomeGalleryTemplate: React.FC<WelcomeGalleryProps> = ({
  ...
  // TODO: Antigravity - Replace with actual config from settings/context
  const [imageGeneratorConfig] = useState<ImageGeneratorConfig>({
    provider: 'free',
    apiKeyConfigured: false // Set to true when API keys are added in settings
  });
  ...
  const handleImageChange = (itemId: string, action: 'upload' | 'generate') => {
    ...
    if (action === 'upload') {
      // TODO: Antigravity - Implement file upload dialog
      // 1. Open file picker
      // 2. Upload to storage
      // 3. Update item.imageUrl
      // 4. Set item.imageSource = 'uploaded'
  ...
  const handleGenerate = (prompt: string, provider: string) => {
    // TODO: Antigravity - Implement actual generation
    // 1. Call appropriate API endpoint based on provider
    // 2. Show loading state
    // 3. On success, update item.imageUrl with generated image URL
    // 4. Set item.imageSource = 'generated'
  ...
  const filteredItems = pageType === 'mixed' 
    ? items 
    : items.filter(item => 
        pageType === 'characters' ? item.category === 'character' : item.category === 'world'
      );
  ...
  return (
    <div className="min-h-screen" style={{ background: '#08080c' }}>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-white/50 mt-2 text-sm md:text-base">
                ...
                {pageType === 'mixed' && 'Characters, worlds, and everything in between'}
              </p>
            </div>
          ...
          {pageType === 'mixed' && (
            <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
              {['All', 'Characters', 'Worlds', 'Factions', 'Locations'].map((filter) => (
                <button
                  key={filter}
                  ...
                  style={{
                    background: filter === 'All' 
                      ? 'linear-gradient(135deg, rgba(249, 115, 22, 0.2) 0%, rgba(234, 88, 12, 0.1) 100%)'
                      : 'rgba(255, 255, 255, 0.03)',
                    border: `1px solid ${filter === 'All' ? 'rgba(249, 115, 22, 0.3)' : 'rgba(255, 255, 255, 0.06)'}`,
                    color: filter === 'All' ? '#F97316' : 'rgba(255, 255, 255, 0.5)'
                  }}
                >
                  {filter}
                </button>
      ...
      <div className="px-6 pb-12 md:px-8 lg:px-12">
        <div className="max-w-7xl mx-auto">
          <div 
            className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5 auto-rows-[minmax(180px,auto)]"
          >
            {/* Create New Card */}
            <CreateNewCard 
              type={pageType === 'worlds' ? 'world' : 'character'} 
              onClick={() => onCreateNew(pageType === 'worlds' ? 'world' : 'character')} 
            />
      ...
      {filteredItems.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 px-6">
          ...
          <p className="text-white/40 text-center max-w-md mb-6">
            Start building your creative universe by creating your first {pageType === 'worlds' ? 'world' : 'character'}.
          </p>
          <button
            onClick={() => onCreateNew(pageType === 'worlds' ? 'world' : 'character')}
